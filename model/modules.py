...
class ScaledDotProductAttention(nn.Module):
    def __init__(self, d_k, dropout: float = .1):
    ...
    def forward(self, q, k, v, attn_mask=None):
        ...
        Args:
            ...
            attn_mask: [b_size x n_heads x len_q x len_k]
        ...
        if attn_mask is not None:
            assert attn_mask.size() == scores.size(), f'attn_mask should have size' \
                                                      f' [b_size x n_heads x len_q x len_k], not {attn_mask.size()}'
