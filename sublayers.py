...
from model.modules import Linear, ScaledDotProductAttention, LayerNormalization
...
class _MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_mask=None):
        ...
        # context: [b_size, n_heads, len_q, d_v],
        # attn: [b_size, n_heads, len_q, len_k]
        context, attn = self.attention(q_s, k_s, v_s, attn_mask=attn_mask)
        # context: [b_size, len_q, n_heads * d_v]
...
class MultiheadAttention(nn.Module):
    def __init__(self, d_k: int, d_v: int, d_model: int, n_heads: int, dropout: float = .1):
