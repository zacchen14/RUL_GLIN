...
from utils import FixedPositionalEncoding, prior_association
...
class RULformer(nn.Module):
    ...
    def __init__(self, d_model: int = 14, d_emb: int = 128, max_life: Optional[int] = None,
        ...
        self.mlp_head = nn.Sequential(
            ...
            nn.Dropout(dropout),
            nn.Linear(32, 1),
            # nn.Sigmoid() # sigmoid is effectiveless, but at association discrepency, to avoid the output is out of boarder,
            # such that the Gaussian kernel generates NaN.
        )
    ...
    def forward(self, src: Tensor, tgt: Tensor, label: Tensor = None,
        ...
        if need_ass_dis is True:
            ...
            sigma = torch.clamp(sigma, 0.25, 50)
            sigma = sigma.cpu()

            if exists(self.max_life):
                prior = torch.stack(
                    [torch.stack(
                        [prior_association(mha_attn.size(-2),
                                           mha_attn.size(-1),
                                           mha_attn.size(-1) - label[i].cpu().detach().numpy() * self.max_life - self.window_size,
                                           sigma[i, j])
